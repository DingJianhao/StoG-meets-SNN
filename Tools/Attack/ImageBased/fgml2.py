...
class FGML2(BaseAttack):
    def __init__(self, model, loss_function, forward_function=None, targeted=False, representation=None,
                 eps=0.007, **kwargs):
        super(FGML2, self).__init__(model, loss_function, forward_function, representation, targeted)
        self.eps = eps
        self.kwargs = kwargs
    ...
    def forward(self, imgs, labels):
        ...
        # print(grad.max())
        g_norms = torch.norm(grad.view(imgs.size(0), -1), p=2, dim=1) + EPS_FOR_DIVISION
        grad = grad / g_norms.view(imgs.size(0), 1, 1, 1)

        adv_images = imgs + self.eps * grad
